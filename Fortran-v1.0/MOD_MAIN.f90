...
! calculate net surface energy flux for cloudy conditions and potential surface melt rate is linearly related to any positive net surface energy flux of a thawing surface
  ...
  ! We also estimate the energy balance of the daily melt and refreezing periods, QMP and Qfair - \00QMP
  ...
  ! debug
    if (debug_switch) then
                 write(*,*) "meltcs",meltcs(debug_lon, debug_lat)
                      write(*,*) "meltFDcs",meltFDcs(debug_lon, debug_lat)
                           write(*,*) "meltFDoc",meltFDoc(debug_lon, debug_lat)
                                  end if
